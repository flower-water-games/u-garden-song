...
namespace Broccoli.Utils
{
    ...
    public class MeshJob {
		...
		/// <summary>
		/// Applies gradient values on the UV1 mapping channel.
		/// x: forward gradient from the base of the mesh to the top (Y axis, values 0-1).
		/// y: side gradient from the center to both sides of the mesh (Z axis, values 0-1).
		/// z: same forward gradient.
		/// w: same side gradient.
		/// </summary>
		/// <param name="mesh">Mesh to apply the UV1 gradient.</param>
		/// <param name="bounds">Bounds of the mesh used to calculate the gradient at each vertex.</param>
		/// <returns>List ov UV1s applied to the mesh.</returns>
		public static List<Vector4> ApplyMeshGradientUV4 (Mesh mesh, Bounds bounds) {
			...
			for (int i = 0; i < totalVertices; i++) {
				...
				uv1s.Add (new Vector4 (fGradient, sGradient, fGradient, sGradient));
			}
			mesh.SetUVs (1, uv1s);
			return uv1s;
		}
		...
		/// <param name="bounds">Bounds of the mesh used to calculate the gradient at each vertex.</param>
		/// <returns>List ov UV1s applied to the mesh.</returns>
		public static List<Vector2> ApplyMeshGradientUV2 (Mesh mesh, Bounds bounds) {
			...
			for (int i = 0; i < totalVertices; i++) {
				fGradient = vertices[i].y / bounds.max.y;
				sGradient = Mathf.Abs (vertices[i].z / bounds.max.z);
				uv2s.Add (new Vector2 (fGradient, sGradient));
			}
			mesh.SetUVs (1, uv2s);
			return uv2s;
		}
